...
export function isCurrentRoute(to: string | UrlOptions, options: CurrentUrlOptions = {}) {
    const { exact = false, current } = options;

    const currentUrl = current || (typeof window !== 'undefined' ? window.location.pathname : '');
    let isCurrent = false;
...
export function useIsCurrentRoute(to: string | UrlOptions, options: CurrentUrlOptions = {}) {
